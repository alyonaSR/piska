...
def test_avt_interface():
    m = AVTModel()
    out = m.predict({"AVT:F30": 61.4, "AVT:T33": 348.2})
...
def test_avt_more_diesel_draw_means_heavier_tail():
    """Больше отбор дизельной фракции -> T95 растёт."""
    m = AVTModel()
    low = m.predict({"AVT:F30": 55.0, "AVT:T33": 348.2})["feed_t95_c"].mean
    high = m.predict({"AVT:F30": 68.0, "AVT:T33": 348.2})["feed_t95_c"].mean
    assert high > low
...
def test_go_hotter_reactor_means_less_sulfur():
    ...
    m = GOModel()
    base = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 0.0, "d_feed_t95_c": 0.0})
    hot = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 2.0, "d_feed_t95_c": 0.0})
...
def test_go_heavier_feed_means_more_sulfur():
    ...
    m = GOModel()
    base = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 0.0, "d_feed_t95_c": 0.0})
    heavy = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 0.0, "d_feed_t95_c": 5.0})
...
def test_bigger_step_widens_interval():
    ...
    m = GOModel()
    small = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 0.5, "d_feed_t95_c": 0.0})
    big = m.predict({"sulfur_anchor": 9.0, "d_go_temp_c": 5.0, "d_feed_t95_c": 0.0})
...
def test_chain_avt_output_feeds_go_input():
    ...
    a = avt.predict({"AVT:F30": 61.4, "AVT:T33": 348.2})
    b = avt.predict({"AVT:F30": 68.0, "AVT:T33": 348.2})
    d_t95 = b["feed_t95_c"].mean - a["feed_t95_c"].mean
    assert d_t95 > 0
    s = go.predict({"sulfur_anchor": 8.5, "d_go_temp_c": 0.0, "d_feed_t95_c": d_t95})
