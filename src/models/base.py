...
EXPECTED_SIGNS: Dict[str, Dict[str, int]] = {
    ...
    # Найдено Person 1 (прогон полного цикла): остаток AVTModel.feed_ebp_c
    # немонотонен по AVT:F30 -- от опорной точки в обе стороны прогноз
    # ПАДАЕТ (356.07 -> 350.62 при -2.5, но и -> 351.66 при +2.5), с
    # обвалом в 7 градусов между +1.0 и +2.5. Похоже на переобучение
    # остатка у края локальной плотности обучающих данных. Формула-baseline
    # уже монотонна по F30 (+2.66463), но AVTModel вообще не заводила
    # monotone_constraints ни для одного из 4 выходов -- остаток был
    # свободен нарушить то, что формула соблюдает. AVT:F30 -- активный
    # рычаг оптимизатора (config/constraints.yaml, active: true), поэтому
    # немонотонность здесь напрямую переворачивает совет от шума датчика.
    "feed_ebp_c": {
        "AVT:F30": +1,  # больше отбор фр.290-350 -> тяжелее хвост -> выше EBP
    },
}
